...
function rvdx_theme_related_posts() {
	...
	$settings = array(
		...
		'block_subtitle'  => 'related_posts_block_subtitle',
		...
		'button_visible'  => 'related_posts_button',
	);
	...
	if ( $holder_view_dir ) {
		...
		$block_title = ( $settings['block_title'] ) ? '<h2 class="entry-title">' . $settings['block_title'] . '</h2>' : '';
		$block_subtitle = ( $settings['block_subtitle'] ) ? '<p class="entry-subtitle">' . $settings['block_subtitle'] . '</p>' : '';
