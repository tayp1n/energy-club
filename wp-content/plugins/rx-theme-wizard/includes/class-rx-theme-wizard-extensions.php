...
if ( ! class_exists( 'Rx_Theme_Wizard_Extensions' ) ) {

	/**
	 * Define Rx_Theme_Wizard_Extensions class
	 */
	class Rx_Theme_Wizard_Extensions {
		...
		public function __construct() {
			...
			add_action( 'rx-theme-wizard/after-plugin-activation', array( $this, 'prevent_bp_redirect' ) );
			add_action( 'rx-theme-wizard/after-plugin-activation', array( $this, 'prevent_elementor_redirect' ) );
			add_action( 'rx-theme-wizard/after-plugin-activation', array( $this, 'prevent_bbp_redirect' ) );
			add_action( 'rx-theme-wizard/after-plugin-activation', array( $this, 'prevent_booked_redirect' ) );
			add_action( 'rx-theme-wizard/after-plugin-activation', array( $this, 'prevent_tribe_redirect' ) );
			add_action( 'rx-theme-wizard/after-plugin-activation', array( $this, 'prevent_woo_redirect' ) );

			add_action( 'rx-theme-wizard/install-finished', array( $this, 'ensure_prevent_booked_redirect' ) );

			if ( rx_theme_wizard()->is_wizard( 4 ) ) {
				add_action( 'rx-theme-wizard/page-after', array( $this, 'ensure_prevent_booked_redirect' ) );
			}

			add_filter( 'rx-theme-wizard/send-install-data', array( $this, 'add_multi_arg' ), 10, 2 );
		...
		public function set_success_redirect_for_theme_wizard() {
			if ( rx_theme_wizard_settings()->get_all_settings() ) {
				...
				add_filter( 'rx-theme-wizard/success-redirect-url', array( $this, 'theme_wizard_success_redirect' ) );
			}
		}
		...
		public function theme_wizard_success_redirect( $redirect ) {
			...
			$redirect = rx_theme_wizard()->get_page_link( array( 'step' => 1, 'advanced-install' => 1 ) );
			$skin     = false;

			if ( rx_theme_wizard_data()->is_single_skin_theme() ) {
				$skin = rx_theme_wizard_data()->get_first_skin();
			}

			if ( false !== $skin && rx_theme_wizard_data()->is_single_type_skin( $skin['skin'] ) ) {
				$redirect = rx_theme_wizard()->get_page_link(
		...
		public function maybe_print_dashboard_css() {
			...
			rx_theme_wizard()->print_inline_css( 'dashboard.css' );
			wp_enqueue_script( 'rx-theme-wizard-dashboard' );
		...
		public function add_dashboard_plugins_section( $builder, $dashboard ) {
			...
			$plugins = rx_theme_wizard_settings()->get( array( 'plugins' ) );
			...
			$builder->register_section(
				array(
					'rx-theme-wizard' => array(
						'title' => esc_html__( 'Recommended plugins', 'tm-dashboard' ),
						'class' => 'tm-dashboard-section tm-dashboard-section--rx-theme-wizard',
			...
			$builder->register_html(
				array(
					'rx-theme-wizard-content' => array(
						'parent' => 'rx-theme-wizard',
						'html'   => $content,
					),
				)
			);
		...
		public function single_plugin_item( $slug, $plugin ) {
			...
			rx_theme_wizard()->get_template( 'dashboard/item.php', $data );
		}
...
/**
 * Returns instance of Rx_Theme_Wizard_Extensions
 *
 * @return object
 */
function rx_theme_wizard_ext() {
	return Rx_Theme_Wizard_Extensions::get_instance();
}
