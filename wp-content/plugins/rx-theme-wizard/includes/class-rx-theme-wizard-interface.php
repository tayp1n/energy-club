...
if ( ! class_exists( 'Rx_Theme_Wizard_Interface' ) ) {

	/**
	 * Define Rx_Theme_Wizard_Interface class
	 */
	class Rx_Theme_Wizard_Interface {
		...
		public function import_tabs_visibility( $is_visible = true ) {
			...
			if ( ! empty( $_GET['referrer'] ) && 'rx-theme-wizard' === $_GET['referrer'] ) {
				return false;
			}
		...
		public function menu_page() {
			...
			add_menu_page(
				esc_html__( 'Theme Installation Wizard', 'rx-theme-wizard' ),
				esc_html__( 'Theme Wizard', 'rx-theme-wizard' ),
				'manage_options',
				rx_theme_wizard()->slug(),
		...
		public function render_plugin_page() {
			...
			rx_theme_wizard()->get_template( 'page-header.php' );
			$this->dispatch();
			rx_theme_wizard()->get_template( 'page-footer.php' );
		}
		...
		public function item_template() {
			...
			if ( empty( $_GET['page'] ) || rx_theme_wizard()->slug() !== $_GET['page'] ) {
				return;
			}
		...
		public function get_item( $slug, $name ) {
			...
			ob_start();
			$wizard_item = rx_theme_wizard()->get_template( 'plugin-item.php' );
			$item = ob_get_clean();
		...
		public function get_loader() {
			ob_start();
			rx_theme_wizard()->get_template( 'loader.php' );
			return ob_get_clean();
		}
		...
		public function dispatch() {
			...
			$dispatch = apply_filters( 'rx-theme-wizard/steps', array(
			...
			do_action( 'rx-theme-wizards/page-before' );

			if ( isset( $dispatch[ $step ] ) ) {
				rx_theme_wizard()->get_template( $dispatch[ $step ] );
			}

			do_action( 'rx-theme-wizards/page-after' );
		...
		public function before_import_title() {
			...
			if ( empty( $skins ) ) {
				esc_html_e( 'No data found for installation', 'rx-theme-wizard' );
			} elseif ( 1 === count( $skins ) ) {
				esc_html_e( 'Start Theme Setup', 'rx-theme-wizard' );
			} else {
				esc_html_e( 'Select skin and start install', 'rx-theme-wizard' );
			}
		...
		public function get_skins() {
			...
			$this->skins = rx_theme_wizard_settings()->get( array( 'skins', 'advanced' ) );
		...
		public function get_skin_data( $key = null ) {
			...
			if ( empty( $this->skin ) ) {
				...
				$data = rx_theme_wizard_settings()->get( array( 'skins', 'advanced', $skin ) );
		...
		public function get_skin_plugins( $slug = null ) {
			...
			$plugins_str = '';
			$format      = '<div class="rx-theme-wizard-skin-plugins__item">%s</div>';
		...
		public function server_notice( $when = 'always' ) {
			...
			$data = array(
				array(
					...
					'rec'     => '5.4',
					'units'   => null,
					'name'    => esc_html__( 'PHP version', 'rx-theme-wizard' ),
				...
				array(
					...
					'rec'     => 128,
					'units'   => 'Mb',
					'name'    => esc_html__( 'Memory limit', 'rx-theme-wizard' ),
				...
				array(
					...
					'rec'     => 60,
					'units'   => 's',
					'name'    => esc_html__( 'Max execution time', 'rx-theme-wizard' ),
					'compare' => array( $this, 'val_compare' ),
				),
			);

			$format     = '<li class="rx-theme-wizard-server__item%5$s">%1$s: %2$s%3$s &mdash; <b>%4$s</b></li>';
			$result     = '';
			$has_errors = false;
			...
			foreach ( $data as $prop ) {
				...
				if ( -1 === $compare ) {
					...
					$msg        = sprintf( esc_html__( '%1$s%2$s Recommended', 'rx-theme-wizard' ), $prop['rec'], $prop['units'] );
					$scs        = '';
					$has_errors = true;

					$this->set_wizard_errors( $prop['arg'] );
				} else {
					$msg = esc_html__( 'Ok', 'rx-theme-wizard' );
					$scs = ' check-success';
				}
			...
			if ( 'always' === $when ) {
				return sprintf( '<ul class="rx-theme-wizard-server">%s</ul>', $result );
			}

			if ( 'errors' === $when && $has_errors ) {
				$message = sprintf(
					'<div class="rx-theme-wizard-server-error">%1$s</div>',
					__( 'Not all of your server parameters met requirements. You can continue the installation process, but it will take more time and can probably drive to bugs:', 'rx-theme-wizard' )
				);
				return sprintf( '%2$s<ul class="rx-theme-wizard-server">%1$s</ul>', $result, $message );
			}
		...
		public function set_wizard_errors( $arg = null ) {
			...
			wp_cache_set( 'errors', $errors, 'rx-theme-wizard' );
		...
		public function get_install_skin_button( $skin = '' ) {
			...
			$url    = rx_theme_wizard()->get_page_link( array( 'step' => 2, 'skin' => $skin ) );
			$label  = esc_html__( 'Select Skin', 'rx-theme-wizard' );
			$format = '<a href="%1$s" data-loader="true" class="btn btn-primary"><span class="text">%2$s</span><span class="rx-theme-wizard-loader"><span class="rx-theme-wizard-loader__spinner"></span></span></a>';

			if ( rx_theme_wizard_data()->is_single_skin_theme() || rx_theme_wizard_data()->is_single_type_skin( $skin ) ) {
				$label  = esc_html__( 'Start Install', 'rx-theme-wizard' );
			}

			if ( rx_theme_wizard_data()->is_single_type_skin( $skin ) ) {
				$next_step = isset( $_GET['advanced-install'] ) && '1' === $_GET['advanced-install'] ? 'configure-plugins' : 3;
				$url = rx_theme_wizard()->get_page_link( array( 'step' => $next_step, 'skin' => $skin, 'type' => 'full' ) );
			}
...
/**
 * Returns instance of Rx_Theme_Wizard_Interface
 *
 * @return object
 */
function rx_theme_wizard_interface() {
	return Rx_Theme_Wizard_Interface::get_instance();
}

rx_theme_wizard_interface();
