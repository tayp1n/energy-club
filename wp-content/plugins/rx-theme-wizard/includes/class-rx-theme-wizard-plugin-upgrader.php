...
class Rx_Theme_Wizard_Plugin_Upgrader extends Plugin_Upgrader {
	...
	public function maybe_adjust_source_dir( $source, $remote_source, $upgrader ) {
		...
		if ( ! rx_theme_wizard_installer()->is_wizard_request() || ! is_object( $wp_filesystem ) ) {
			return $source;
		}
		...
		if ( ! empty( $subdir_name ) && $subdir_name !== $desired_slug ) {
			...
			if ( true === $wp_filesystem->move( $from_path, $to_path ) ) {
				...
				return new WP_Error(
					'rename_failed',
					esc_html__( 'The remote plugin package does not contain a folder with the desired slug and renaming did not work.', 'rx-theme-wizard' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'rx-theme-wizard' ),
			...
			return new WP_Error(
				'packaged_wrong',
				esc_html__( 'The remote plugin package consists of more than one file, but the files are not packaged in a folder.', 'rx-theme-wizard' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'rx-theme-wizard' ),
				array( 'found' => $subdir_name, 'expected' => $desired_slug )
			);
		}
