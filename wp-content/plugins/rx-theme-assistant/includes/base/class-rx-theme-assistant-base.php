...
abstract class Rx_Theme_Assistant_Base extends Widget_Base {
	...
	public function __get_global_template( $name = null ) {
		...
		if ( ! $template ) {
			$template = rx_theme_assistant()->get_template( $this->get_name() . '/global/' . $name . '.php' );
		}
	...
	public function __get_render_template( $name = null ) {
		return rx_theme_assistant()->get_template( $this->get_name() . '/render/' . $name . '.php' );
	}
	...
	public function __get_edit_template( $name = null ) {
		return rx_theme_assistant()->get_template( $this->get_name() . '/edit/' . $name . '.php' );
	}
	 ...
	 * @return void
	 */
	public function __open_wrap( $class = '' ) {
		printf( '<div class="elementor-%s rx-theme-assistant-addons %s">', $this->get_name(), $class );
	}
