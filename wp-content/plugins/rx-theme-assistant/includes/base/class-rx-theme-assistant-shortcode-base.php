 ...
 */
if ( ! class_exists( 'Rx_Theme_Assistant_Shortcode_Base' ) ) {

	abstract class Rx_Theme_Assistant_Shortcode_Base {
		...
		public function get_template( $name ) {
			...
			if ( filter_var( $this->atts['use_custom_template'], FILTER_VALIDATE_BOOLEAN ) && ! empty( $this->atts['custom_template'] ) ) {

				$template = rx_theme_assistant()->get_template( $this->get_tag() . '/' . $this->atts['custom_template'] . '/' . $name . '.php' );
			...
			return rx_theme_assistant()->get_template( $this->get_tag() . '/global/' . $name . '.php' );
		}
