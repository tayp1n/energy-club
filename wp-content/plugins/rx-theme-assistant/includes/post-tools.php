...
if ( ! class_exists( 'Rx_Theme_Assistant_Post_Tools' ) ) {

	/**
	 * Define Rx_Theme_Assistant_Post_Tools class
	 */
	class Rx_Theme_Assistant_Post_Tools {
		...
		public function get_post_content( $args = array(), $type = 'post', $id = 0 ) {
			...
			if ( filter_var( $args['visible'], FILTER_VALIDATE_BOOLEAN ) ) {
				if ( 'term' === $type ) {
					...
					$post_id = rx_theme_assistant_tools()->is_blog() ? get_option('page_for_posts') : null ;
		...
		public function get_post_comment_count( $args = array(), $id = 0 ) {
			...
			if ( empty( $object->ID ) || ! post_type_supports( $post_type, 'comments' )) {
				return false;
			}
		...
		public function get_post_image( $args = array(), $type = 'post', $id = 0 ) {
			...
			$default_args = array(
				...
				'html'                   => '<a href="%1$s" %2$s ><img src="%3$s" %6$s alt="%4$s" %5$s ></a>',
				'class'                  => 'wp-image',
				'placeholder'            => true,
				'placeholder_src'        => '',
			...
			$size         = in_array( $args['size'], $intermediate_image_sizes ) ? $args['size'] : 'post-thumbnail';
			...
			if ( $thumbnail_id ) {
				...
				$image_src_set = wp_get_attachment_image_srcset( $thumbnail_id, $size );
				$html_src_set  = $image_src_set ? 'srcset="' . $image_src_set . '"' : '' ;
				$src           = $image_data[0];
				...
				if ( $args['placeholder_src'] ) {
					$src = $args['placeholder_src'];
				} else {
			...
			if ( isset( $src ) ) {
				$html = sprintf( $args['html'], esc_url( $link ), $class, esc_url( $src ), esc_attr( $alt ), $html_tag_suze, $html_src_set );
			}
...
/**
 * Returns instance of Rx_Theme_Assistant_Post_Tools
 *
 * @return object
 */
function rx_theme_assistant_post_tools() {
	return Rx_Theme_Assistant_Post_Tools::get_instance();
}
